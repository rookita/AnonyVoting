...
@app.route("/voterwe/we_upload_pk", methods=["POST"])
def we_upload_pk():
    data = request.get_data()
    js = json.loads(data)
    username = js.get("name", 0)
    ...
    try:
        call_contract(
                vote_addr,"VoteWe","updatepubkey", args=[userid, epub, epub], signer=signer
            )
...
@app.route("/voterwe/upload_wePK", methods=["POST"])
def upload_wePK():
    data = request.get_data()
    js = json.loads(data)
    PK = js.get("PK", 0)
...
# 权重投票
@app.route("/voterwe/add_we_ballot", methods=["GET", "POST"])
def add_weballot():
    data = request.get_data()
    js = json.loads(data)
    username = js.get("name", 0)
    ...
    weight = js.get("weight")
    spub = js.get("spub")

    try:
        call_contract(
            vote_addr,"VoteWe","addBallot", args=[ballot_str, signature, userid, weight, spub], signer=signer
            )
    except Exception as e:
        print(e)
        return jsonify({"result": 'error3'})
...
# 上传share，C1，C2
@app.route("/voterwe/upload_share", methods=["POST"])
def upload_share():
    data = request.get_data()
    js = json.loads(data)
    ...
    try:
        call_contract(
                vote_addr,"VoteWe","setShare", args=[userid, share], signer=signer
            )
    except Exception as e:
        print(e)
        return jsonify({"result": 'error2'})
...
# 获取结果
@app.route("/voterwe/get_we_result", methods=["POST"])
def get_we_result():
    ...
    return jsonify({"result":'success',"vote":"we","para":Para, "C2":C2, "Cx":allshare})
