...
class client_config:
    ...
    remote_rpcurl = (
        "http://127.0.0.1:8545"  # 采用rpc通信时，节点的rpc端口,和要通信的节点*必须*一致,如采用channel协议通信，这里可以留空
    )
    ...
    account_keyfile_path = (
        "bin/accounts"  # 保存keystore文件的路径，在此路径下,keystore文件以 [name].keystore命名
    )
    ...
    solc_path = "/home/ubuntu/python-sdk/bin/solc/v0.4.25/solc"
