...
type Polynom struct {
	// coeff are the coefficients(系数) of the polynom. coeff[i] is the coefficient of x^i.
	coeff []*big.Int
...
func NewRandom(degree int64, modulus *big.Int) Polynom {
	...
	var (
		err   error
		coeff = make([]*big.Int, degree) //开辟内存
	)
	for i := range coeff {
...
func Interpolate(x0 *big.Int, x []*big.Int, y []*big.Int, modulus *big.Int) (y0 *big.Int) {
	...
	for i := range x {
		nums[i] = product(x, x0, i)   //分子
		dens[i] = product(x, x[i], i) //分母
	}
...
func Interpolate1(prime *big.Int, x []*big.Int, y *big.Int, i int) (v *big.Int) {
	num := product(x, big.NewInt(0), i)
	den := product(x, x[i], i)
	num.Mul(num, y)
	num.Mod(num, prime)
	v = divmod(num, den, prime)
	if v == nil {
		return nil // x values are not distinct.
	}
	v.Mod(v, prime)
